...
public abstract class BasePage {
    ...
    protected OrangeWait oranWait() {
        return new OrangeWait();
    }
    ...
    public void enterText(By loc, String text, boolean clearFirst) {
        enterText(oranWait().For(loc), text, clearFirst);
    }
    ...
    public void click(WebElement ele, boolean expectRefresh) {
        ...
        if (expectRefresh) {
            oranWait().untilElementNotPresent(body);
        }
        oranWait().waitUntilStable();
    }
    ...
    public Optional<WebElement> findElementBy(By locator, Predicate<WebElement> filter) {
        List<WebElement> matches = oranWait().ForMany(locator, false).stream().filter(filter).collect(toList());
        switch (matches.size()) {
            case 0:
                return Optional.empty();
            case 1:
    ...
    public String getText(By locator) { return getText(oranWait().For(locator)); }
    ...
    public List<String> getTexts(By locator) {
        return getTexts(oranWait().ForMany(locator, false));
    }
